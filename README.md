...
- الدالة تنفّذ **طلب HTTP فعلي** للرابط (مع تتبع التحويلات وحد أقصى)، **تمنع هجمات SSRF** على شبكة الخادم (تجاهل IPs خاصة وأضرار شائعة)، وتُرجع **`is_safe` / `confidence` / `reasons`** حسب تحليل مبني على الاستجابة الفعلية (مثلاً HTTP غير مشفّر مقابل HTTPS، أخطاء الشبكة، تغيّر النطاق بعد التحويل).
